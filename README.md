...
ZeroQ 센서 수집 및 제어 서버입니다. HTTP와 MQTT를 통해 `sensorId` 기준 raw telemetry/heartbeat를 저장하고, 게이트웨이 runtime heartbeat/snapshot, 명령 생성/전송/ACK, 데드레터 관리를 수행합니다.

## 역할

- 센서 장치 런타임 projection 관리
- telemetry, heartbeat, batch ingest 처리
- gateway runtime heartbeat/history 및 latest snapshot 관리
- 품질 판정과 최근 수집 모니터링
...
- `/api/zeroq/v1/sensor/ingest`
  - gateway heartbeat: `/api/zeroq/v1/sensor/ingest/gateway-heartbeat`
- `/api/zeroq/v1/sensor/commands`
- `/api/zeroq/v1/sensor/monitoring`
  - 센서별 최근 telemetry: `/api/zeroq/v1/sensor/monitoring/sensors/{sensorId}/telemetry/recent`
...
- ingest와 command pull/ack 일부 경로는 `GATEWAY` 역할도 허용합니다.
- 공간 스냅샷은 더 이상 이 모듈이 소유하지 않고, `zeroq-back-service`가 admin 센서 매핑 + raw telemetry를 조합해 계산합니다.
- 게이트웨이 runtime 값은 `gateway_heartbeat`, `gateway_status_snapshot`에 저장되고, `zeroq-back-service`가 admin gateway metadata와 조합해 화면에 노출합니다.
